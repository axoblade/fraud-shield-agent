...
| **AI / ML**        | google-generativeai SDK, Gemini Function Calling, Heuristic Fallback Engine |
...
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Gemini 2.5 Flash** | Multi-turn reasoning engine; the agent's brain. Gemini receives 7 registered tools, decides which to call based on transaction context, analyses results, and submits a final risk decision. All via function calling with the `google-generativeai` Python SDK. |
...
│                                          │
│  Turn 1: fetch_account_history (always)  │
│  Turn 2+: Gemini decides; velocity?     │
│           mule? baseline? balance?        │
│  Final:  submit_risk_decision            │
│          → risk score 0–100              │
...
| ----------- | --------------------------------------------- | --------------------------------------------------------------- |
| Agent Brain | Google Gemini 2.5 Flash with function calling | Multi-turn reasoning and dynamic tool selection                 |
...
1. Calls `fetch_account_history`; always the first step
...
- **Replay engine**; Stream mixed fraud + normal transactions chronologically at configurable speed (1× to 20×)
...
```bash
git clone <repo-url> && cd fraud-shield-agent
...
fraud-shield-agent/
├── agent/
│   ├── agent_core.py          # Multi-turn agent with Gemini function calling
...
│   └── verify_day1.py         # Post-load verification
│
...
`Google Gemini 2.5 Flash` · `MongoDB Atlas` · `MongoDB MCP Server` · `FastAPI` · `React 19` · `TypeScript` · `Vite` · `WebSocket` · `Docker` · `Google Cloud Run` · `PaySim Dataset` · `Python 3.12`
